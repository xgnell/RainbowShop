...
$return_path = "/public/templates/account/update-account.php";
...
");
if ((mysqli_num_rows($customer_phone) == 1 && mysqli_fetch_array($customer_phone)['id'] != $customer_id)) {
    display_signup_notification_page(
        false,
        $notification_title,
...
");
if ((mysqli_num_rows($customer_email) == 1 && mysqli_fetch_array($customer_email)['id'] != $customer_id)) {
    display_signup_notification_page(
        false,
        $notification_title,
        "Email đã tồn tại",
...
sql_cmd("
    ...
    SET
        ...
        birth='{$customer["birth_year"]}-{$customer["birth_month"]}-{$customer["birth_day"]}',
