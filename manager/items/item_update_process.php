<?php
    ...
    sql_query("
        UPDATE items
        SET name = '$item_name', picture = '$item_picture_name', price = $item_price, description = '$item_description', id_type = $item_type_id, color = '$item_color'
        WHERE id = $item_id;
    ");
    ...
    foreach ($item_sizes as $item_size_name => $item_size_amount) {
        foreach ($item_meta_data as $item_data) {
            $found = false;
            if ($item_data["size"] == $item_size_name) {
                ...
                if ($item_size_amount == 0) {
                    ...
                    sql_query("
                        ...
                        WHERE id_item = $item_id AND size = '$item_size_name';
                    ");
                } else {
                    // Update size's amount value
                    sql_query("
                        UPDATE item_meta_data
