...
<body>
    ...
    <div class="page-body">
        ...
        <div class="page-content">
            <!-- Item update form -->
            <form action="/manager/items/item_update_process.php" method="POST" enctype="multipart/form-data">
                ...
                Type: <select name="type" id="display-type">
                    ...
                    <?php
                        ...
                        foreach($item_types as $item_type) {
                            ?>
                            <option value="<?= $item_type['name'] ?>" <?php if($item_type['id'] == $item['id_type']) echo "selected" ?> ><?= $item_type['name'] ?></option>;
                            <?php
                        }
                    ?>
                </select><br>
                Color: <select name="color" id="display-color">
                    ...
                    <?php
                        ...
                        foreach ($item_colors as $item_color) {
                            ?>
                            <option value="<?= $item_color['color'] ?>" <?php if ($item_color['color'] == $item['color']) echo "selected" ?> ><?= $item_color['color'] ?></option>
                            <?php
                        }
                    ?>
                </select><br>
                                ...
                                <td>
                                    <?php
                                        ...
                                        foreach ($item_meta_data as $item_data) {
                                            if ($item_data["size"] == $item_size["size"]) {
                                                $found = true;
                                                echo "<input name=\"size-{$item_size["size"]}\" type=\"number\" value=\"{$item_data["amount"]}\">";
                                                break;
                                            }
                                        }
                                        if (!$found) {
                                            echo "<input name=\"size-{$item_size["size"]}\" type=\"number\" value=\"0\">";
                                        }
                                    ?>
                                </td>
                                <?php
